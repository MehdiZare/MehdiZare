  ...
  return (
    <footer className="border-t border-warm-gray bg-paper">
      <div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-8 px-6 py-12 sm:flex-row sm:items-center lg:px-8">
        <div>
          <p className="font-mono text-sm font-medium tracking-wide text-ink">{siteName}</p>
          <p className="mt-0.5 font-mono text-xs tracking-wide text-mid-gray">{credentialLine}</p>
          <p className="mt-1 text-sm text-mid-gray">{locationLine}</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-5">
          {socialLinks.map((link) => (
            <a
              ...
              className="flex items-center gap-2 text-sm text-mid-gray transition-colors hover:text-ink"
